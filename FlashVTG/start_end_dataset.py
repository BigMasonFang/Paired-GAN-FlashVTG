...
class StartEndDataset(Dataset):
    ...
    def load_data(self):
        ...
        if self.data_ratio != 1:
            ...
            logger.info("Using {}% of the data: {} examples".format(
                self.data_ratio * 100, n_examples))
        return datalist
    ...
    def _load_model_inputs(self, meta):
        ...
        if self.dset_name in ['tvsum']:
            ...
            if len(model_inputs["saliency_all_labels"]) != len(
                ...
                model_inputs["video_feat"] = model_inputs[
                    "video_feat"][:len(model_inputs["saliency_all_labels"])]
            ...
            if "relevant_windows" in meta:  ## For Qvhighlights test set
                model_inputs["span_labels"] = self.get_span_labels(
                    meta["relevant_windows"], ctx_l)  # (#windows, 2)
                if self.dset_name in ['charadesSTA', 'tacos',
                                      'activitynet']:  ## charades, tacos, nlq
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs["saliency_all_labels"] = \
    ...
    def get_saliency_labels_sub_as_query(self,
                                         gt_window,
                                         duration,
                                         ctx_l,
                                         max_n=2):
    ...
    def get_saliency_labels_all(self,
                                rel_clip_ids,
                                scores,
                                ctx_l,
                                max_n=1,
    ...
    def get_saliency_labels_all_tvsum(self,
                                      labels,
                                      ctx_l,
                                      max_n=1,
                                      add_easy_negative=False):

        agg_scores = np.sum(labels - np.ones_like(labels),
                            axis=-1)[:ctx_l]  # start from 1, so minus 1
        score_array = agg_scores / 80 * 12
        sort_indices = np.argsort(agg_scores)  # increasing

        hard_pos_clip_indices = [
            min(idx, ctx_l - 1) for idx in sort_indices[-max_n:]
        ]
        hard_neg_clip_indices = [
            min(idx, ctx_l - 1) for idx in sort_indices[:max_n]
        ]
    ...
    def get_saliency_labels_all_youtube(self,
                                        labels,
                                        ctx_l,
                                        max_n=1,
                                        add_easy_negative=False):
    ...
    def _get_query_feat_by_qid(self, qid):
        if self.dset_name == 'tvsum':
            q_feat = np.load(join(self.q_feat_dir,
                                  "{}.npz".format(qid)))  # 'token', 'text'
        ...
        # youtube-hl
            ...
            try:
                ...
                q_feat = np.load(q_feat_path)[self.q_feat_type].astype(
                    np.float32)
                if self.q_feat_type == "last_hidden_state":
                    q_feat = q_feat[:self.max_q_l]
                if self.normalize_t:
    ...
    def random_drop_rows(self, embeddings):
        ...
        if num_drop_rows > 0:
            row_indices = np.random.choice(len(embeddings),
                                           size=num_drop_rows,
                                           replace=False)
        ...
        if self.dset_name == 'tvsum':
            ...
            for _feat_dir in self.v_feat_dirs:
                try:
                    ...
                    _feat_rgb = np.load(_feat_path)[:self.max_v_l].astype(
                        np.float32)

                    _feat_path = join(_feat_dir, f"{vid}_opt.npy")
                    _feat_opt = np.load(_feat_path)[:self.max_v_l].astype(
                        np.float32)
                    ...
                    try:
                        ...
                        _feat = np.load(_feat_path)[:self.max_v_l].astype(
                            np.float32)
                    except:
                        _feat_path = join(_feat_dir, f"{vid}.npz")
                        _feat = np.load(
                            _feat_path)["features"][:self.max_v_l].astype(
                                np.float32)
            ...
            for _feat_dir in self.v_feat_dirs:
                try:
                    ...
                    try:
                        ...
                        _feat = torch.load(
                            _feat_path)[:self.max_v_l].float().numpy()
                    except:
                        _feat_path = join(_feat_dir, f"{vid}.npy")
                        _feat = np.load(_feat_path)[:self.max_v_l].astype(
                            np.float32)
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        ...
        if k == "saliency_all_labels":
            pad_data, mask_data = pad_sequences_1d([e[1][k] for e in batch],
                                                   dtype=np.float32,
                                                   fixed_length=None)
        ...
        if k == 'vid':
            batched_data[k] = [e[1][k] for e in batch]
            continue
        batched_data[k] = pad_sequences_1d([e[1][k] for e in batch],
                                           dtype=torch.float32,
                                           fixed_length=None)
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    if "saliency_all_labels" in batched_model_inputs:
        targets["saliency_all_labels"] = batched_model_inputs[
            "saliency_all_labels"].to(device, non_blocking=non_blocking)
