...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--use_neg",
                            action="store_true",
                            help="use negative samples in training")
        parser.add_argument("--dset_name",
                            type=str,
                            choices=[
                                "hl", 'tvsum', 'charadesSTA', 'tacos',
                                'youtube_uni', 'qv_internvideo2',
                                'charadesSTA_internvideo2'
                            ])
        parser.add_argument(
            "--dset_domain",
            type=str,
            help=
            "Domain to train for tvsum dataset. (Only used for tvsum and youtube-hl)"
        )
        ...
        parser.add_argument(
            ...
            default="val",
            help=
            "should match keys in video_duration_idx_path, must set for VCMR")
        parser.add_argument(
            "--debug",
            action="store_true",
            help=
            "debug (fast) mode, break all loops, do not load all data into memory."
        )
        parser.add_argument(
            "--data_ratio",
            type=float,
            default=1.0,
            help=
            "how many training and eval data to use. 1.0: use all, 0.1: use 10%."
        ...
        parser.add_argument("--drop_last",
                            type=bool,
                            default=False,
                            help="train_loader config")
        parser.add_argument("--eval_bsz",
                            type=int,
                            default=100,
        ...
        parser.add_argument("--eval_untrained",
                            action="store_true",
        ...
        parser.add_argument(
            ...
            default=None,
            help=
            "checkpoint path to resume or evaluate, without --resume_all this only load weights"
        )
        parser.add_argument(
            "--resume_all",
            action="store_true",
        ...
        parser.add_argument("--resume_adapter",
                            type=str,
                            default=None,
                            help="checkpoint path to resume adapter")
        ...
        parser.add_argument(
            ...
            default=None,
            help=
            "Evaluating during training, for Dev set. If None, will only do training, "
        )
        parser.add_argument("--no_norm_vfeat",
                            action="store_true",
                            help="Do not do normalize video feat")
        parser.add_argument("--no_norm_tfeat",
                            action="store_true",
                            help="Do not do normalize text feat")
        parser.add_argument(
            "--v_feat_dirs",
            type=str,
            nargs="+",
            help=
            "video feature dirs. If more than one, will concat their features. "
            "Note that sub ctx features are also accepted here.")
        parser.add_argument("--t_feat_dir",
                            type=str,
        ...
        parser.add_argument("--t_feat_dim",
                            type=int,
        ...
        parser.add_argument("--q_feat_type",
                            type=str,
                            default="last_hidden_state",
                            help="use video features")
        ...
        parser.add_argument('--t2v_layers',
                            default=2,
                            type=int,
        ...
        parser.add_argument("--use_txt_pos",
                            action="store_true",
        ...
        parser.add_argument('--total_prompts',
                            default=10,
                            type=int,
        ...
        parser.add_argument("--n_input_proj",
                            type=int,
                            default=2,
        ...
        parser.add_argument(
            ...
            choices=['l1', 'ce'],
            help=
            "l1: (center-x, width) regression. ce: (st_idx, ed_idx) classification."
        )
        parser.add_argument('--sample_radius', default=1.5, type=float)
        parser.add_argument("--lw_reg",
                            type=float,
                            default=0.2,
                            help="weight for span loss, set to 0 will ignore")
        parser.add_argument("--lw_cls",
                            type=float,
                            default=1.,
                            help="weight for span loss, set to 0 will ignore")
        parser.add_argument("--lw_sal",
                            type=float,
                            default=0.1,
                            help="weight for span loss, set to 0 will ignore")
        parser.add_argument(
        ...
        parser.add_argument("--conf_thd",
                            type=float,
                            default=0.0,
                            help="only keep windows with conf >= conf_thd")
        parser.add_argument("--nms_thd",
                            type=float,
                            default=0.7,
        ...
        parser.add_argument("--nms_type",
                            type=str,
                            default="normal",
                            choices=["normal", "linear"])
        parser.add_argument('--lambda_gan',
                            type=float,
                            default=0.1,
                            help='Weight for adversarial loss')
        parser.add_argument('--add_gan',
                            type=bool,
                            default=False,
                            help='Enable GAN training')
        parser.add_argument('--gan_dis_type',
                            type=int,
                            default=1,
                            help='Choose GAN discriminator type')
        parser.add_argument('--gan_dis_loss',
                            type=int,
                            default=1,
                            help='Choose GAN discriminator loss')
        parser.add_argument('--mask_type',
                            type=str,
                            default='gaussian',
                            help='Choose GAN span mask')
        parser.add_argument('--mix_saliency',
                            type=bool,
                            default=False,
                            help='In MR Choose use saliency togther')
        self.parser = parser
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            ctx_str = opt.ctx_mode + "_sub" if any(
                ["sub_ctx" in p for p in opt.v_feat_dirs]) else opt.ctx_mode
            opt.results_dir = os.path.join(
                opt.results_root, "-".join([
                    opt.dset_name, ctx_str, opt.exp_id,
            ...
            save_fns = ['FlashVTG/model.py', 'FlashVTG/transformer.py']
            for save_fn in save_fns:
                shutil.copyfile(
                    save_fn,
                    os.path.join(opt.results_dir, os.path.basename(save_fn)))
        ...
        opt.device = torch.device(f"cuda:{opt.device}" if opt.device >=
                                  0 else "cpu")
...
class TestOptions(BaseOptions):
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            ...
            default=None,
            help=
            "dir to save results, if not set, fall back to training results_dir"
        )
        self.parser.add_argument(
            "--model_dir",
            type=str,
            help=
            "dir contains the model file, will be converted to absolute path afterwards"
        )
